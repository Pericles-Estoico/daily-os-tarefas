...
import { Clock, Search, CheckCircle2, AlertCircle, Plus } from 'lucide-react';
...
export default function Rotina() {
  ...
  const filteredTasks = todayTasks.filter((t) => t.title.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleComplete = () => {
    if (selectedTask) {
      ...
      completeRoutineTask(selectedTask.id, evidencia ? [evidencia] : [], owners[0]?.id || '');
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {filteredTasks.map((task) => (
          <Card key={task.id} className={`cursor-pointer hover:border-primary/50 ${task.critical ? 'border-red-500/30' : ''}`} onClick={() => setSelectedTask(task)}>
            <CardContent className="p-3 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">{getMarketplaceName(task.marketplaceId)}</p>
              </div>
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                {selectedTask.status !== 'DONE' && (
                  <div>
                    <Label>Evidência {selectedTask.evidenceRequired && <span className="text-red-500">*</span>}</Label>
